...
export class EngineerAgent {
    ...
    run(t) {
        ...
        const warnings = [];
        if (dp > 0.3)
            warnings.push(`Pressure drop ${dp.toFixed(2)} exceeds 0.3 in.wc - energy penalty`);
        if (vel_sup > 1400)
            warnings.push(`Supply velocity ${vel_sup} fpm causes noise (>65 dB)`);
        if (vel_ret > 1000)
            warnings.push(`Return velocity ${vel_ret} fpm exceeds recommended 1000 fpm`);
        if (reynoldsNumber < 2300)
            warnings.push("Laminar flow detected - may cause uneven distribution");
        if (flowRegime === "transitional")
            warnings.push("Transitional flow - unstable performance");
        if (heatGain_tons > 0.5)
            warnings.push(`Heat gain ${heatGain_tons.toFixed(2)} tons affects efficiency`);
        // Efficiency rating
