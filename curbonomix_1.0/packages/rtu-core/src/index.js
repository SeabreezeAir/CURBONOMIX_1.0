...
exports.sizePlenum = sizePlenum;
exports.buildAdapter = buildAdapter;
exports.toDXF = toDXF;
exports.toGCode = toGCode;
exports.toSubmittal = toSubmittal;
/* --- MODEL CATALOG (extend as needed) --- */
const catalog = {
    // examples (inches). Add your factory models here.
...
function resolveModel(model) {
    ...
    if (!hit)
        return undefined;
    return { model: key, ...hit };
}
/* --- aero/thermo sizing (toy, deterministic) --- */
function sizePlenum(spec) {
    const A_ft2 = (spec.new_L * spec.new_W) / 144 / 6; // crude fraction of plan area
    const v_sup = 1200, v_ret = 900;
    const cfm_s = spec.cfm_supply ?? Math.round(Math.max(0.5, A_ft2) * v_sup);
    const cfm_r = spec.cfm_return ?? Math.round(Math.max(0.5, A_ft2) * v_ret);
    const dp_inwc = +(0.08 * (cfm_s / 1000) ** 2).toFixed(3);
    return { cfm_s, cfm_r, dp_inwc, vel_sup_fpm: v_sup, vel_ret_fpm: v_ret };
}
/* --- geometry (rectangular frustum) --- */
function buildAdapter(spec) {
    const L1 = spec.new_L, W1 = spec.new_W;
    const L2 = spec.new_L2 ?? spec.new_L, W2 = spec.new_W2 ?? spec.new_W;
    const H = spec.height;
    const v = [
...
function toSubmittal(spec, perf) {
