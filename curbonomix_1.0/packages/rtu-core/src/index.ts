export type RTUSpec = {
  model?: string;
  new_L: number; new_W: number; height: number; flange_h: number;
  supply_x: number; supply_y: number; return_x: number; return_y?: number;
  steel_gauge: number; sst: number; brake_lim: number;
  new_L2?: number; new_W2?: number; cfm_supply?: number; cfm_return?: number;
};
...
export function resolveModel(model: string){ const k=model.trim().toUpperCase(); const hit=catalog[k]; return hit?{model:k,...hit}:undefined; }
export function sizePlenum(spec:RTUSpec){
...
export function toSubmittal(spec:RTUSpec, perf:ReturnType<typeof sizePlenum>){
